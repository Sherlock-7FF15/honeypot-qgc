...
If your running container still uses an older `sandbox-run.sh` and disconnects immediately, force-patch it:

```bash
./scripts/force_patch_nosuid_fallback.sh ssh-shadow
```
